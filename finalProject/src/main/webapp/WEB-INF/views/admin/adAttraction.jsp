        ...
        <div class="board-theme">
            <div class="search-btn" id="searchBtn">
                ...
                <div class="search-section">
                	<form action="attractionSearch.ad" method="get">
                	...
                	<c:choose>
                		<c:when test="${keyword != null}">
		                	<c:choose>
		                		...
		                		<c:otherwise>
		                			 <li class="page-item"><a class="page-link" href="attractionSearch.ad?currentPage=${pi.currentPage - 1}&type=${type}&keyword=${keyword}">&lt;</a></li>
		                		</c:otherwise>
		                	</c:choose>
		                    ...
		                    <c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage}">
		                    	<c:choose>
		                    		<c:when test="${p eq pi.currentPage}">
		                   				<li class="page-item disabled"><a class="page-link" href="attractionSearch.ad?currentPage=${p}&type=${type}&keyword=${keyword}">${p}</a></li>
			                		</c:when>
			                		<c:otherwise>
			                			<li class="page-item"><a class="page-link" href="attractionSearch.ad?currentPage=${p}&type=${type}&keyword=${keyword}">${p}</a></li>
			                		</c:otherwise>
		                    	</c:choose>
		                    </c:forEach>
                		...
                		<c:otherwise>
		                	<c:choose>
		                		...
		                		<c:otherwise>
		                			 <li class="page-item"><a class="page-link" href="attraction.ad?currentPage=${pi.currentPage - 1 }">&lt;</a></li>
		                		</c:otherwise>
		                	</c:choose>
		                    ...
		                    <c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage}">
		                    	<c:choose>
		                    		<c:when test="${p eq pi.currentPage}">
		                   				<li class="page-item disabled"><a class="page-link" href="attraction.ad?currentPage=${p}">${p}</a></li>
			                		</c:when>
			                		<c:otherwise>
			                			<li class="page-item"><a class="page-link" href="attraction.ad?currentPage=${p}">${p}</a></li>
			                		</c:otherwise>
		                    	</c:choose>
		                    </c:forEach>
...
<script>
    ...
    $(function () {
		$(".theme-table>tbody>tr>td").not(":first-child").click(function () {
			...
			location.href = 'attractionSelect.ad?boardNo='+bno;
		});
	});
