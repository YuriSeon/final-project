...
<body>
	...
	<form action="themeUpdate.ad" method="post" enctype="multipart/form-data">
	<input type="hidden" name="boardWriter" value="${loginUser.nickname}">
        <div class="the-area" >
            <div class="the_title">테마 수정</div>
            <div class="the_text">
                <div class="the_input" style="margin-top: 10px;">
                    ...
                    <select name="themeNo" id="themaCategory" style="margin-left: 47px;">
                        ...
                        <c:choose>
                    		<c:when test="${t.themeNo eq 1}"><option value="1" selected>별보러 가지 않을래?</option></c:when>
                    		<c:otherwise><option value="1">별보러 가지 않을래?</option></c:otherwise>
                    	</c:choose>
                        <c:choose>
                    		<c:when test="${t.themeNo eq 2}"><option value="2" selected>소원을 말해봐</option></c:when>
                    		<c:otherwise><option value="2">소원을 말해봐</option></c:otherwise>
                    	</c:choose>
                    	<c:choose>
                    		<c:when test="${t.themeNo eq 3}"><option value="3" selected>평생 친구와 함께</option></c:when>
                    		<c:otherwise><option value="3">평생 친구와 함께</option></c:otherwise>
                    	</c:choose>
                    	<c:choose>
                    		<c:when test="${t.themeNo eq 4}"><option value="4" selected>계곡</option></c:when>
                    		<c:otherwise><option value="4">계곡</option></c:otherwise>
                    	</c:choose>
                    	<c:choose>
                    		<c:when test="${t.themeNo eq 5}"><option value="5" selected>힐링</option></c:when>
                    		<c:otherwise><option value="5">힐링</option></c:otherwise>
                    	</c:choose>
                    	<c:choose>
                    		<c:when test="${t.themeNo eq 6}"><option value="6" selected>전시회</option></c:when>
                    		<c:otherwise><option value="6">전시회</option></c:otherwise>
                    	</c:choose>
                    </select>
                </div>
                <div class="the_input">
                ...
                <div class="the_input">
                    ...
                    <input type="text" id="infoTime" name="dayOff" style="margin-left: 60px;" value="${i.dayOff}">
                </div>
                
                <div class="the_input">
                    <label for="address">주소</label>
                    <input type="text" id="address" onclick="postcode();" style="margin-left: 72px;" name="infoAddress" value="${i.infoAddress}">
                ...
                <div class="the_input">
                    ...
                    <textarea name="boardContent" id="boardContent" cols="70" rows="10" style="margin-left: 41px; resize: none;" required>${b.boardContent}</textarea>
                </div>
